prepare_bn <- function(background.network = NULL,
  ...
  reacs2 <- paste0(background.network$gene_source, "=", background.network$gene_target)
  ...
  returnList <- list()
  returnList[[length(returnList)+1]] <- background.network[complete.cases(background.network), ]
