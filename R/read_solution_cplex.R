read_solution_cplex <- function(variables = variables,
  ...
  for(ii in 2:(length(cplexSolution)-1)){
    ...
    if(length(idxReac)>0){
      ...
      reactions <- sapply(strsplit(x = variables$var_exp[which(variables$var%in%varvar[idxReac])], split = " ", fixed = TRUE), '[', 2)
      interactions <- sapply(strsplit(x = variables$var_exp[which(variables$var%in%varvar[idxReac])], split = " ", fixed = TRUE), '[', 4)
      
      uInt <- unique(interactions)
      currSIF <- matrix(data = , nrow = length(uInt), ncol = 4)
      currSIF[, 1] <- sapply(strsplit(x = uInt, split = "=", fixed = TRUE), '[', 1)
      currSIF[, 2] <- "1"
      currSIF[, 3] <- sapply(strsplit(x = uInt, split = "=", fixed = TRUE), '[', 2)
      for(jj in 1:length(uInt)){
        
        idx <- which(grepl(pattern = paste0(" ", uInt[jj]), x = variables$var_exp[which(variables$var%in%varvar[idxReac])], fixed = TRUE))
  ...
  for(ii in 1:nrow(combSIF)){
    ...
    for(jj in 1:length(sifAll)){
      ...
      if(length(idx)>0){
        ...
        ud <- c(ud, strsplit(x = sifAll[[jj]][idx, 4], split = "; ", fixed = TRUE)[[1]])
