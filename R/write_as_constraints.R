write_as_constraints <- function(background.network = background.network,
  ...
  if(is.null(pValThresh)){
    ...
    if(length(idx)==0){
      ...
      as_reactions <- paste0("reaction ", temp$pfam_source, "=", temp$pfam_target, " of ",
                             temp$gene_source, "=", temp$gene_target)

      cc <- paste0(variables$var[which(variables$var_exp%in%as_reactions)], " = 0")
