write_loop_constraints <- function(variables = variables,
  ...
  # print("Writing Constraints 6/6..")

  sif <- unique(background.network[, c("gene_source", "gene_target")])

  species <- unique(c(sif[, 1], sif[, 2]))

  speciesVar <- variables$var[grepl(pattern = "node",
                                    x = variables$var_exp)]
  speciesExp <- variables$var_exp[grepl(pattern = "node",
                                        x = variables$var_exp)]
  reacVar <- variables$var[grepl(pattern = "interaction",
                                 x = variables$var_exp)]
  distVar <- variables$var[grepl(pattern = "dist ",
                                 x = variables$var_exp)]
  ...
  for(ii in seq_len(nrow(sif))){
    ...
    var1 <- variables$var[which(variables$var_exp==paste0("dist ",
                                                          sif[ii, 2]))]
    var2 <- variables$var[which(variables$var_exp==paste0("dist ",
                                                          sif[ii, 1]))]
    var3 <- variables$var[which(variables$var_exp==paste0("interaction ",
                                                          sif[ii, 1],
                                                          "=",
                                                          sif[ii, 2]))]
