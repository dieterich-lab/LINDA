read_solution_cbc <- function(variables = variables,
                              condition = 1){

  solution <- read.csv(file = paste0("solution_", condition, ".csv"))
  rownames(solution) <- solution$name
  res <- solution[paste0("xb", 1:nrow(solution)), ]
