write_objective_function <- function(background.network = background.network, 
  ...
  if(!is.null(pValThresh2)){
    ...
    idx <- intersect(x = which(background.network$source_score!=0), y = which(background.network$source_fdr<=pValThresh2))
    if(length(idx)>0){
      obj <- rep("", length(idx))
    ...
    idx <- intersect(x = which(background.network$target_score!=0), y = which(background.network$target_fdr<=pValThresh2))
    if(length(idx)>0){
      obj <- rep("", length(idx))
      for(ii in 1:length(idx)){
