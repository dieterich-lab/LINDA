...
LINDA can use alternative-splicing information in two ways through the
```constraints_mode``` argument. The default is ```constraints_mode = "hard"```,
which keeps the original AS-constraint formulation and forces significantly
affected reactions to zero according to ```pValThresh``` and
```splice_effect_sign```. Alternatively, ```constraints_mode = "soft"``` keeps
AS-affected reactions feasible but penalizes them in the objective function. The
default ```splice_effect_sign``` is now ```"negative"```, while
```"positive"``` and ```"both"``` remain available.

```{r, message=FALSE}
res <- runLINDA(input.scores = input.scores, as.input = as.input, 
                background.network = bg, solverPath = "~/Downloads/cplex", 
                input.node = NULL, constraints_mode = "hard", 
                pValThresh = 0.05, splice_effect_sign = "negative", 
                top = 2, lambda1 = 10, lambda2 = 0.001, 
                mipgap = 0.001, relgap = 0.001)
